...
const CookiePolicyScreen = () => {
  ...
  return (
    <SafeAreaView style={[styles.safeArea, isWeb && styles.webSafeArea]}>
      <Navbar
        title={menuStrings.cookies}
        isRTL={isRTL}
        onBack={() => navigation.navigate('Home')}
        backLabel={strings.tabs.home}
      />
      <ScrollView contentContainerStyle={[styles.content, isWeb && styles.webContent]}>
        <Text style={[styles.title, isRTL && styles.rtlText]}>{menuStrings.cookies}</Text>
        <Text style={[styles.paragraph, isRTL && styles.rtlText]}>{menuStrings.cookiesDescription}</Text>
      </ScrollView>
    </SafeAreaView>
  );
};
